...
## MCSTEST.MCS — the format validation song
The disk now carries **MCSTEST.MCS**, generated by
[`tools/make_test_mcs.py`](../tools/make_test_mcs.py). It packs every notation element we
decode into one song — all note and rest durations (and beamed notes), mid-measure
sharp/flat/natural, a key signature, a dotted note, chords, an empty measure, both
clefs, and an 8va staff. Load it in MCS (`LOAD MCSTEST`) and check, measure by measure,
that what the program draws and plays matches what `mcs_convert/mcs/reader.py` decodes
(the generator prints the decoded expectation). Regenerate + re-inject any time:
```
python tools/make_test_mcs.py           # writes samples/MCSTEST.MCS
python -c "import sys; sys.path.insert(0,'tools'); from make_mcs_dsk import *; \
  img=open('samples/MCS_boot_360k.img','rb').read(); \
  open('samples/MCS_boot_360k.img','wb').write( \
  add_file_to_image(img,'MCSTEST.MCS',open('samples/MCSTEST.MCS','rb').read()))"
```
