...
def _emit_staff(bars, treble: bool):
    ...
    for bar in bars:
        ...
        for tick, advance, midis, ties in bar:
            for base, dotted in _decompose(advance, allow_dot=bool(midis)):
                x = min(31, tick)
                if not midis:
                    entries.append(make_entry(_REST_SYM[base], rest_v, x))
                else:
                    for midi in sorted(midis, reverse=True):
                        ...
                        if acc_state.get(v, key_state_default) != acc:
                            ...
                            entries.append(make_entry(sym, v, x))
                            acc_state[v] = acc
                        entries.append(make_entry(_NOTE_SYM[base], v, x))
                        if dotted:
                            entries.append(make_entry(_SYM_DOT, v, min(31, x + 1)))
                    ...
                    for m in piece_ties:            # one tie glyph per tied member,
                        ...
                        entries.append(make_entry(_SYM_TIE, tv, min(31, x + 1)))
...
def encode_song(song: Song, *, bar_ticks: int = 32, tempo_byte0: int = 0x80,
                split: int = 67) -> bytes:
    """Song (32nd-note ticks) -> .MCS bytes. `split` is the treble/bass boundary."""
    treble_ev, bass_ev = [], []
    total = 0
    ...
    tre = _emit_staff(_staff_slots(treble_ev, total, bar_ticks), treble=True)
    bas = _emit_staff(_staff_slots(bass_ev, total, bar_ticks), treble=False)
